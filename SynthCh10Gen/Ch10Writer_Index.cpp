...
void ClCh10Writer_Index::Init(int iHandle, unsigned int uChanID, uint8_t nodesPerRoot, Ch10Format_Index* formatter)
{
  Ch10Writer::Init(iHandle, uChanID);

  this->formatter = formatter;
  this->nodesPerRoot = nodesPerRoot;
...
void ClCh10Writer_Index::AppendMsg() {
  AppendNodeIndex();
}


void ClCh10Writer_Index::Commit() {
  WriteNodePacket();

  if (rootNodeCount == nodesPerRoot) {
    WriteRootPacket();
    rootNodeCount = 0;
  }
...
void ClCh10Writer_Index::AppendNodeIndex()
{
  __int64             uOffset;
  ...
  Ch10Format_Index::RequiredIndexFields formatData = formatter->GetFormatValues();

  enI106Ch10GetPos(iHandle, &uOffset);

  memset(&suNodeInfo, 0, sizeof(SuIndex_NodeMsg));
  memcpy(suNodeInfo.suTime.suRtcTime.aubyRefTime, formatData.refTime, 6);
  suNodeInfo.lOffset = uOffset - formatData.packetLen;
  suNodeInfo.suData.uChannelID = formatData.chID;
