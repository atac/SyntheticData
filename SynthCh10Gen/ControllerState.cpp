...
ControllerStatus ControllerState::Configure(string configFilepath) {
...
void ControllerState::InitControllerObjects() {
  // init structures
...
int ControllerState::InitOutputFile(string directory, string filename) {
...
void ControllerState::InitTimers() {
  for (auto t : *timers) {
    t->InitToSimClock();
  }
}
...
void ControllerState::AddIndexChannel(ClCh10Writer_Time* timeWriter) {
...
ClCh10Writer_Time* ControllerState::AddTimeChannel() {
...
ControllerStatus ControllerState::AddDataChannel(ConfigChannel config) {  // FOREACH SOURCE
...
ClSimTimer* ControllerState::GetTimer(Rate rate, bool startExpired) {
...
ClSimTimer* ControllerState::GetExistingTimer(int64_t rtcTimeout, bool startExpired) {
  for (auto t : *timers) {
...
ClSimTimer* ControllerState::CreateTimer(int64_t timeout, bool startExpired) {
...
void ControllerState::InsertTimer(ClSimTimer* timer) {
  bool inserted = false;
...
std::string ControllerState::GenerateChannelName(Ch10Channel::ChannelType type) {
...
Ch10Channel* ControllerState::CreateChannel(
  Ch10Writer* writer,
...
ClSource_Nav* ControllerState::CreateSource(ConfigDataSource config, int channelID)
{
...
Ch10Formatter_PCM* ControllerState::CreatePcmFormatter(Ch10Channel::ChannelDataFormat format, Rate framerate, ClSource_Nav* src) {
...
Ch10Formatter_1553* ControllerState::Create1553Formatter(Ch10Channel::ChannelDataFormat format, ClSource_Nav* src)
{
...
Ch10Formatter_Video* ControllerState::CreateVideoFormatter(Ch10Channel::ChannelDataFormat format, ClSource_Nav* src)
{
